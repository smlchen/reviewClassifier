...
#code adpated from roc_curves.py to plot pr ccurves instead
...
def plotROC(fpr, tpr, auc, tpr_upper = None, tpr_lower = None, file_name = None):
    ...
    plt.title('Doc2Vec Logistic Rgression Model')
    plt.legend(loc='upper right')
    if file_name: plt.savefig(r'./pr_plots/' + file_name + '.png')
    plt.show()


def get_pr_auc(y_test, y_predict):
    fpr = dict(); tpr = dict(); roc_auc = dict(); avg_precision = dict()
    for rating in range(5):
        fpr[rating], tpr[rating], _ = precision_recall_curve(y_test[:, rating], y_predict[:, rating])
        roc_auc[rating] = 0 #sklearn.metrics.auc(fpr[rating], tpr[rating])
    return fpr, tpr, roc_auc


def getPRForAllFolds(best_model):
    ...
    for fold in range(1, 6):
        ...
        fpr, tpr, auc = get_pr_auc(y_test, y_predict)
        auc_list.append(auc)
...
if __name__ == '__main__':
    #model = 'best_tfidf_model.h5'
    #model = 'best_doc2vec_model.h5'
    #model = 'best_mse_tfidf_model.h5'
    #model = 'best_mse_doc2vec_model.h5' 
    #model = 'tfidf_logreg_model.pkl'
    model = 'doc2vec_logreg_model.pkl'
    getPRForAllFolds(model)
